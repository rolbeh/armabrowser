...
class DB
{
    ...
    public static function GetUsedAddons(){
        
        $sql = 'SELECT 
                asb_addon_keys.addon_key_pubkhash as addon_pubkhash
                ,`client_addon_name` as addon_name
                ,`client_addon_displayname` as addon_displayname
                ,`client_addon_displaytext` as addon_displaytext
                ,`client_addon_version` as addon_version
                , MAX(client_addon_lastsighting) as addon_lastsighting
                FROM asb_client_addon 
                INNER JOIN asb_client ON asb_client_addon.client_id=tbl_client.client_id
                INNER JOIN asb_client_addon_keys ON asb_client_addon.client_addon_id = asb_client_addon_keys.client_addon_id
                INNER JOIN asb_addon_keys on asb_client_addon_keys.addon_keys_id = asb_addon_keys.addon_keys_id
                WHERE asb_client.client_isdebug is null
                group by 
                `client_addon_name`
                ,`client_addon_displayname`
                ,`client_addon_displaytext`
